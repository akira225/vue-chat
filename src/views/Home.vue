    ...
    ConnectUser : function () {
      if (this.UserName) {
        ...
        STORE_.state.Connection.onopen = function () {
          STORE_.state.Connection.send(JSON.stringify({Command:"ADD_USER", User:STORE_.state.UserName}))
          STORE_.state.Connection.send(JSON.stringify({Command:"GET_USERS"}))
        ...
        STORE_.state.Connection.onmessage = function (response) {
          ...
          if(responseJson.Command === "REMOVE_USER"){
            STORE_.state.Users = STORE_.state.Users.filter(user => {return user !== responseJson.User})
          }
          if(responseJson.Command === "GET_USERS" && !STORE_.state.isValidUsers){
